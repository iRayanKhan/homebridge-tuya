    ...
    for (const device of this.config) {
      if (device.ip) {
        ...
        try {
          ...
          device.productKey = tuyaDevice.device.productKey;

          this.log.info('Device %s has IP %s', device.id, device.ip);

          this.emit('device-new', device);
        } catch (error) {
          this.log.error('%s was unable to be found. Please try using a static IP in your config.json.', device.id,error);
