...
class Curtain {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    // Add stop movement button
    if (!platform.homebridgeAccessories.get(UUIDGen.generate(this.deviceConfig.id + this.deviceConfig.name + 'stop'))) {
      this.stopButtonAccessory = new PlatformAccessory(this.deviceConfig.name + ' Stop', UUIDGen.generate(this.deviceConfig.id + this.deviceConfig.name + 'stop'), Service.Switch);
      platform.registerPlatformAccessory(this.stopButtonAccessory);
    } else {
      this.stopButtonAccessory = platform.homebridgeAccessories.get(UUIDGen.generate(this.deviceConfig.id + this.deviceConfig.name + 'stop'));
    }
    ...
    // Button service
    this.stopService = this.stopButtonAccessory.getService(Service.Switch);

    if (this.stopService) {
      this.stopService.setCharacteristic(Characteristic.Name, this.deviceConfig.name + ' Stop');
    } else {
      this.stopService = this.stopButtonAccessory.addService(Service.Switch, this.deviceConfig.name + ' Stop');
    }

    this.stopService.getCharacteristic(Characteristic.On)
      .on('get', callback => { callback(null, false); })
      .on('set', callback => {
        this.device.set({dps: this.dps.toString(), set: this.stopValue.toString()}).then(result => {
          this.stopService.setCharacteristic(Characteristic.On, false);
          callback(null);
