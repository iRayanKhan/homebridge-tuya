...
class Curtain {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    // Default time to open
    if ('timeToOpen' in this.deviceConfig.options) {
      this.timeToOpen = this.deviceConfig.options.timeToOpen;
    } else {
      this.timeToOpen = 30;
    }
  ...
  setTargetPosition(position, callback) {
    ...
    // Stop moving for percentage-based moves
    const currentPositionInS = rangeMap(this.lastPosition, 0, 100, -(this.timeToOpen / 2), (this.timeToOpen / 2))
    const targetPositionInS = rangeMap(this.currentTargetPosition, 0, 100, -(this.timeToOpen / 2), (this.timeToOpen / 2));
    const timeToMoveMS = (targetPositionInS - currentPositionInS) * 1000;

    this.timeout = setTimeout(() => {
      this.device.set({dps: this.dps.toString(), set: this.stopValue.toString()});
