...
class Curtain {
  ...
  getCurrentPosition(callback) {
    ...
    this.device.on('data', data => {
      if ('dps' in data && data.dps[this.dps]) {
        console.log('asoidhfsaiohfasdiofdsah', data.dps[this.dps]);
      } else {
        this.device.get();
      }
    });
  ...
  setTargetPosition(position, callback) {
    ...
    console.log('asdfasdfadsf', this.deviceConfig.options)
    const setTo = (moveUp ? this.deviceConfig.options.upValue : this.deviceConfig.options.downValue);

    this.device.set({dps: this.dps, set: setTo}).then(result => {
