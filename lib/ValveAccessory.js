    ...
    _registerCharacteristics(dps) {
        ...
        if (!this.noTimer) {
            service.getCharacteristic(Characteristic.SetDuration)
                ...
                .on('change', (data)=> {
                    console.log("[TuyaAccessory] Water Valve Time Duration Set to: " + data.newValue/60 + " Minutes")
                    ...
                    if(service.getCharacteristic(Characteristic.InUse).value) {
                        ...
                        this.timer = setTimeout( ()=> {
                            console.log("[TuyaAccessory] Water Valve Timer Expired. Shutting OFF Valve");
                ...
                .on('change', (data) => {
                        switch(data.newValue) {
                            case 0:
                                ...
                                console.log("[TuyaAccessory] Water Valve is OFF!");
                                break;
                            case 1:
                                this.lastActivationTime = (new Date()).getTime();                               
                                ...
                                console.log("[TuyaAccessory] Water Valve Turning ON with Timer Set to: "+  this.setDuration/60 + " Minutes");
                                clearTimeout(this.timer); // clear any existing timer
                                this.timer = setTimeout(()=> {
                                    console.log("[TuyaAccessory] Water Valve Timer Expired. Shutting OFF Valve");
            ...
            if (dps[this.dpPower]) {
                ...
                console.log("[TuyaAccessory] Water Valve is ON After Restart. Setting Timer to: "+  this.setDuration/60 + " Minutes");	
                clearTimeout(this.timer); // clear any existing timer								
                this.timer = setTimeout(()=> {
                        console.log("[TuyaAccessory] Water Valve Timer Expired. Shutting OFF Valve");
                        // use 'setvalue' when the timer ends so it triggers the .on('set'...) event
