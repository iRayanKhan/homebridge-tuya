...
class DimmerAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    this.device.on("data", function(data, command) {
      if (data !== undefined) {
        this.log.debug("Updating", this.homebridgeAccessory.displayName, data, command);
        for (var index in data.dps) {
          switch (index) {
            case deviceConfig.dpsOn:
              this.homebridgeAccessory.getService(this.homebridgeAccessory.context.type)
                .getCharacteristic(Characteristic.On).updateValue(data.dps[index]);
              break;
            case deviceConfig.dpsBright:
              this.homebridgeAccessory.getService(this.homebridgeAccessory.context.type)
                .getCharacteristic(Characteristic.Brightness).updateValue(Math.round(data.dps[index] / 255 * 100));
              break;
          }
        }
      } else {
    ...
    this.device.on("error", function(data) {
