...
class SwitchAccessory extends BaseAccessory {
    ...
    _verifyCachedPlatformAccessory() {
        ...
        //we add backlight if available - object 16
        if (this.accessory.getServiceByUUIDAndSubType(Service.Switch, 'switch ' + "16") != undefined){
            console.log("This switch has a backlight, we add it as a button");
            let service = this.accessory.getServiceByUUIDAndSubType(Service.Switch, 'switch ' + "16");
            if (service) this._checkServiceName(service, this.device.context.name + ' backlight');
            else service = this.accessory.addService(Service.Switch, this.device.context.name + ' ' + "backlight", 'switch ' + "16");
