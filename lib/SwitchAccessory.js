...
class SwitchAccessory extends BaseAccessory {
    ...
    _verifyCachedPlatformAccessory(_dps) {
        if (this._justRegistered) return;

        const {Service} = this.hap;
        //get switchcount from dps
        let switchCount = 0;
        ...
        for (let i in _dps){
            if (parseInt(_dps[i]) < 7){//max gangs is 6
                switchCount++;
            }
        }
        ...
        console.log(`switchCount: ${switchCount}, backlightId: ${backlightId}`);
        ...
        console.log(typeof(_dps[15]));
        console.log(typeof(_dps[backlightId]));
        //if (this.accessory.getServiceByUUIDAndSubType(Service.Switch, 'switch ' + backlightId) != undefined){
        if (typeof(_dps[backlightId]) != undefined){
    ...
    _registerCharacteristics(dps) {
        this._verifyCachedPlatformAccessory(dps);
