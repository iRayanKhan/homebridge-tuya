...
class SimpleBlindsAccessory extends BaseAccessory {
    ...
    setTargetPosition(value, callback) {
        this.log.debug('[Tuya] Blinds asked to move from ' + this.assumedPosition + ' to ' + value);

        if (this.changeTimeout) clearTimeout(this.changeTimeout);
        this.targetPosition = value;

        if (this.changeTime !== false) {
            this.log.debug("[Tuya] Blinds " + (this.assumedState === BLINDS_CLOSING ? 'closing' : 'opening') + " had started " + this.changeTime + "; " + (Date.now() - this.changeTime) + 'ms ago');
            const disposition = ((Date.now() - this.changeTime) / (10 * this.duration));
            if (this.assumedState === BLINDS_CLOSING) {
                this.assumedPosition = BLINDS_OPEN - disposition;
            } else {
            ...
            this.log.debug("[Tuya] Blinds' adjusted assumedPosition is " + this.assumedPosition);
        }
