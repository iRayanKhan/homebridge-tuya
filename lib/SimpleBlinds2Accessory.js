...
class SimpleBlinds2Accessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.targetPosition = this.previousPosition = this.currentPosition = this._getCurrentPosition(dps[this.dpPosition]);
        ...
        this.device.on('change', changes => {
            if (changes.hasOwnProperty(this.dpControl) && this.commandedState !== changes[this.dpControl]) {
                ...
                this.log.debug("[TuyaAccessory] SimpleBlinds2 " + this.device.context.name + " commanded to " + this.commandedState);
                ...
                this.positionUpdateTimeout = setTimeout(() => {
                    this.getCurrentPosition((unused, position) => {
                        ...
                        this.log.debug("[TuyaAccessory] SimpleBlinds2 " + this.device.context.name + " resetting position to " + position + " after timeout.");
                ...
                this.log.debug("[TuyaAccessory] SimpleBLinds2 " + this.device.context.name + " saw change to " + changes[this.dpPosition]);
                ...
                this.log.debug("[TuyaAccessory] SimpleBlinds2 " + this.device.context.name + " skipping message (likely due to recent target change): " + JSON.stringify(changes));
            }
        });
    }
    ...
    getTargetPosition(callback) {
        process.nextTick(() => {
            if (this.commandedState == "stop" && this.currentPosition == this.previousPosition) {
                this.log.debug("[TuyaAccessory] SimpleBlinds2 stopped setting to target to current Position");
    ...
    setTargetPosition(value, callback) {
        this.log.debug('[TuyaAccessory] SimpleBlinds2 ' + this.device.context.name + ' target position set to ' + value);
